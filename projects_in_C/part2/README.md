...
1. What was the bug?
	A segmentation fault from an invalid read accessing memory outside of our range.
2. What line did the bug occur?
	line 15 in seg.c.
3. What is the fix for the bug?
	instead of calling mov on the memory address with garbage data we could call sub to 
	make it a vaild memory location to read. 
...
Pro:
1. It examines all possible execution paths and variable values not just the ones being invoked during execution which can reveal errors that might not manifest for months or years after
   release. This makes it especially valuable in terms of reducing the cost of fixing the errors as vulnerabilities can be found early in the development life of the program.

Con:
1. This form of analysis will not find you defects in the runtime environment and can be time consuming if analysis is conducted manually. 
...
Pro:
1. Dynamic analysis reveals any subtle defects or vulnerabilites that are too complex for be picked up by static analysis which makes it very useful for finding and debugging errors.

Con:
1. A downside of dynamic analysis is that it cannot guarantee full test coverage of source code and can provide a false sense of security until a defect that appears once the 
   program has been shipped to users/market and is encountering errors.  
